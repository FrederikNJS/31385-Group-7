...
int
read_linesensor_raw(int no)
{
    return out.line_sensor->data[no];
}

int
read_linesensor_calibrated(int no)
{
    int tmp =
	(100 * (read_linesensor_raw(no) - calibration.ls_black[no])) /
	(calibration.ls_white[no] - calibration.ls_black[no]);
    if(tmp > 100)
	{
	    tmp = 100;
	}
...
int
find_line_position(int line_color, double lineLocations[2])
{
    int i = 0;
    ...
    for(i = 0; i < LINESENSOR_N; i++)
	{
		...
		{
		    sensor[i] =
			100 - read_linesensor_calibrated((LINESENSOR_N - 1) -
							 i);
		}
	    else
		{
		    sensor[i] =
			read_linesensor_calibrated((LINESENSOR_N - 1) - i);
		}
	    temp = sensor[i];
	    if(temp < value1)
		{
    ...
    if(start1 <= 0 && end1 >= LINESENSOR_N - 1)
	{
